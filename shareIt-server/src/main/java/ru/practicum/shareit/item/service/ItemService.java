...
public class ItemService {
    ...
    public ItemDto createItem(Long userId, ItemDto itemDto) {
        if (itemDto.getRequestId() != null) {
            return createItemOnRequest(userId, itemDto, itemDto.getRequestId());
        } else {
    ...
    private ItemDto createItemOnRequest(Long userId, ItemDto itemDto, Long requestId) {
        User owner = findUserById(userId);
        ItemRequest itemRequest = requestRepository.findById(requestId).orElseThrow(() -> new NotFoundException("Запрос не найдено."));
        Item item = ItemMapper.mapToItem(itemDto);
        item.setOwner(owner);
